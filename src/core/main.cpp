...
extern "C" {
	bool RpNormMapPluginAttach(void);
	RwTexture* RpNormMapMaterialGetNormMapTexture(const RpMaterial* material);
	RpMaterial* RpNormMapMaterialSetNormMapTexture(RpMaterial* material, RwTexture* normalmap);
	bool RpNormMapAtomicIsInitialized(const RpAtomic* atomic);
}
...
{
	dbglog("myPluginAttach: called");
	return PipelinePluginAttach ? PipelinePluginAttach() : 0;
}
...
///		InjectHook(0x74639B, selectVM, PATCH_JUMP);
		...
		// DISABLED FOR TESTING: Attach normal map plugin for vehicle/vegetation normal maps
		...
		// DISABLED FOR TESTING: Hook normal map pipeline creation to capture normal map textures
