...
struct Config {
	...
	// SSS Post-Process (kept for future, disabled by default)
	RwBool sssPostProcessEnable;
	float sssPostProcessStrength;
	float sssPostProcessRadius;
	float sssPostProcessThreshold;
...
void pipeUploadPBR(float p0, float p1, float p2, float p3, float p4, float p5, float p6);
