...
CCustomCarEnvMapPipeline__CustomPipeRenderCB_Env(RwResEntry *repEntry, void *object, RwUInt8 type, RwUInt32 flags)
{
	...
	// PBR material defaults (env map pipeline)
	// c22 = {glossiness, specular, specTint, envFresnel}
	// c23 = {wheelFlag, noiseScale, edgeBlend, 0}
	float pbrGloss = 0.9f;
	float pbrSpec = 0.04f;
	float pbrTint = 0.0f;
	float pbrFresnel = config->envFresnel;
	pipeUploadPBR(pbrGloss, pbrSpec, pbrTint, pbrFresnel,
	              (float)renderingWheel, 0.0f, 0.0f);
